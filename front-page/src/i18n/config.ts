...
export function getAvailableLanguages(): string[] {
  ...
  try {
    ...
    return files
      .filter(file => {
        // Match JSON files with language codes (2+ letters, e.g., en.json, es.json, corp.json, pirate.json)
...
export function getLangFromUrl(url: URL): Language {
  ...
  // Only consider it a language if it's in our actual language list
  // This prevents paths like /docs from being treated as languages
...
export function getStaticPaths() {
  const languages = getAvailableLanguages();
  // Exclude 'en' from dynamic paths since English uses the default routes
  // and Starlight needs /en/ for documentation
  return languages
    .filter(lang => lang !== 'en')
    .map((lang) => ({ 
      params: { lang } 
